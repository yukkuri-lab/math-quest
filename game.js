...
class GameController {
    ...
    generateMathProblem(level) {
        ...
        if (type === 'water') {
            wordQuestions = [
                { id: 101, q: `${enemyName}が 18m もぐったあと、7m ういてきた。いま 水面からなんm？`, a: 11 },
                { id: 102, q: `${enemyName}の いる 湖の ふかさが 25m。あと 9m で 底につく。いま 何m？`, a: 16 },
                { id: 103, q: `${enemyName}が 水しぶきを 14かい あげた。さらに 7かい あげた。ぜんぶで？`, a: 21 },
                { id: 104, q: `${enemyName}の ウロコを 13まい 見つけた。あとで 6まい 見つけた。あわせて？`, a: 19 },
                { id: 105, q: `${enemyName}を さがして 船が 20台 出た。そのうち 6台が ひきかえした。残りは？`, a: 14 }
            ];
        } else if (type === 'sky') {
            wordQuestions = [
                { id: 201, q: `${enemyName}が 空を 18m とんだあと、7m おりてきた。あわせて 何m 動いた？`, a: 25 },
                { id: 202, q: `${enemyName}が 14回 羽ばたいたあと、さらに 8回 バタバタさせた。ぜんぶで？`, a: 22 },
                { id: 203, q: `${enemyName}の 羽根が 15まい 落ちていた。あとで 8まい 見つけた。あわせて？`, a: 23 },
                { id: 204, q: `${enemyName}が 空から 20m おりてきた。あと 6m で 地面につく。はじめの高さは？`, a: 26 },
                { id: 205, q: `${enemyName}の 飛ぶスピードが 16あがったあと、さらに 9あがった。あわせて？`, a: 25 }
            ];
        } else if (type === 'shadow') {
            wordQuestions = [
                { id: 301, q: `${enemyName}の あやしい光が 13かい 点滅した。あとで 8かい 光った。あわせて？`, a: 21 },
                { id: 302, q: `${enemyName}の ノイズ音が 16秒間 なった。さらに 9秒 なった。あわせて？`, a: 25 },
                { id: 303, q: `${enemyName}の しゃしんを 12まい とった。5まい 心霊しゃしんだった。普通のしゃしんは？`, a: 7 },
                { id: 304, q: `なぞのUMA ${enemyName} のウワサが 15こ あった。今日 8こ ふえた。ぜんぶで？`, a: 23 },
                { id: 305, q: `${enemyName}から 20m 逃げたあと、うしろをふりむくと 6m さきにいた。ちかばに きたキョリは？`, a: 14 }
            ];
        } else { // beast (森・獣人)
            wordQuestions = [
                { id: 401, q: `${enemyName}の おおきな 足あとが 15こ あった。さらに 8こ 見つかった。ぜんぶで？`, a: 23 },
                { id: 402, q: `${enemyName}の なきごえが 14かい きこえた。あとから 7かい きこえた。あわせて？`, a: 21 },
                { id: 403, q: `${enemyName}から 20m にげた。そこから 6m すすんできた。いま はなれてるキョリは？`, a: 14 },
                { id: 404, q: `${enemyName}が 森の木を 12本 たおした。さらに 7本 たおした。あわせて？`, a: 19 },
                { id: 405, q: `${enemyName}の おおきさが 16m だと 言われていたが、ほんとうは 9m 小さかった。ほんとうの 大きさは？`, a: 7 }
            ];
        }
